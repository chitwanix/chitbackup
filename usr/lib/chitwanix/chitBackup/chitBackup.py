...
class MintBackup:
    ...
    ''' New MintBackup '''
    def __init__(self):
        self.glade = '/usr/lib/chitwanix/chitBackup/chitBackup.glade'
        self.wTree = gtk.glade.XML(self.glade, 'main_window')
        self.wTree.get_widget("main_window").set_icon_from_file("/usr/lib/chitwanix/chitBackup/icon.png")
        ...
        # file extensions chitBackup specific
        ...
        self.wTree.get_widget("image_backup_data").set_from_file("/usr/lib/chitwanix/chitBackup/backup-data.svg")
        self.wTree.get_widget("image_restore_data").set_from_file("/usr/lib/chitwanix/chitBackup/restore-data.svg")
        self.wTree.get_widget("image_backup_software").set_from_file("/usr/lib/chitwanix/chitBackup/backup-software.svg")
        self.wTree.get_widget("image_restore_software").set_from_file("/usr/lib/chitwanix/chitBackup/restore-software.svg")
        ...
        self.wTree.get_widget("label_caption_software_backup2").set_markup("<i><span foreground=\"#555555\">" + _("The list below shows the packages you added to Chitwanix OS") + "</span></i>")
    ...
    def about_callback(self, w):
        ...
        dlg.set_program_name("chitBackup")
        ...
        try:
            ...
            print detail
        try:
            version = commands.getoutput("/usr/lib/chitwanix/common/version.py chitbackup")
            dlg.set_version(version)
        except Exception, detail:
            print detail

        dlg.set_authors(["Arun Kumar Pyasi <root@chitwanix.com>"])
        dlg.set_icon_from_file("/usr/lib/chitwanix/chitBackup/icon.png")
        dlg.set_logo(gtk.gdk.pixbuf_new_from_file("/usr/lib/chitwanix/chitBackup/icon.svg"))
        def close(w, res):
    ...
    def forward_callback(self, widget):
        ...
        if(sel == 1):
            ...
            self.operating = True
            thread = threading.Thread(group=None, target=self.backup, name="chitBackup-copy", args=(), kwargs={})
            thread.start()
        elif(sel == 4):
            # show info page.
            self.wTree.get_widget("button_forward").hide()
            ...
            if(not self.restore_source or self.restore_source == ""):
                ...
                return
            thread = threading.Thread(group=None, target=self.prepare_restore, name="chitBackup-prepare", args=(), kwargs={})
            thread.start()
        elif(sel == 7):
            # start restoring :D
            ...
            self.operating = True
            thread = threading.Thread(group=None, target=self.restore, name="chitBackup-restore", args=(), kwargs={})
            thread.start()
        elif(sel == 8):
            ...
            thr = threading.Thread(group=None, name="chitBackup-packages", target=self.load_packages, args=(), kwargs={})
            thr.start()
            self.wTree.get_widget("button_forward").hide()
            self.wTree.get_widget("button_apply").show()
        elif(sel == 11):
            ...
            if f is None:
                ...
                self.package_dest = f
                self.operating = True
                thr = threading.Thread(group=None, name="chitBackup-packages", target=self.backup_packages, args=(), kwargs={})
                thr.start()
        elif(sel == 14):
            thr = threading.Thread(group=None, name="chitBackup-packages", target=self.load_package_list, args=(), kwargs={})
            thr.start()
        elif(sel == 15):
            inst = False
            ...
            if(not inst):
                ...
                thr = threading.Thread(group=None, name="chitBackup-packages", target=self.install_packages, args=(), kwargs={})
                thr.start()
    ...
    ''' Creates a .chitbackup file (for later restoration) '''
    def create_backup_file(self):
        self.description = "chitBackup"
        desc = self.wTree.get_widget("entry_desc").get_text()
        if(desc != ""):
            self.description = desc
        try:
            of = os.path.join(self.backup_dest, ".chitbackup")
            out = open(of, "w")
            lines = [  "source: %s\n" % (self.backup_dest),
                                    "destination: %s\n" % (self.backup_source),
                                    "file_count: %s\n" % (self.file_count),
    ...
    def backup(self):
        ...
        if(comp[1] is not None):
            ...
            try:
                ...
                chitfile = os.path.join(self.backup_dest, ".chitbackup")
                tar.add(chitfile, arcname=".chitbackup", recursive=False, exclude=None)
            except Exception, detail:
                print detail
    ...
    ''' prepare the restore, reads the .chitbackup file if present '''
    def prepare_restore(self):
        if(self.restore_archive):
            ...
            try:
                ...
                chitfile = self.tar.getmember(".chitbackup")
                if(chitfile is None):
                    ...
                    mfile = self.tar.extractfile(chitfile)
            ...
            self.conf = mINIFile()
            try:
                mfile = os.path.join(self.restore_source, ".chitbackup")
    ...
    def restore(self):
        ...
        if(self.restore_archive):
            ...
            for record in self.tar.getmembers():
                if(not self.operating):
                    break
                if(record.name == ".chitbackup"):
                    # skip chitbackup file
                    continue
                gtk.gdk.threads_enter()
    ...
    def load_packages(self):
        ...
        try:
            ...
            bl = open("/usr/lib/chitwanix/chitBackup/software-selections.list", "r")
